...
const intlMessages = defineMessages({
  autoPlayWarning: {
    id: 'app.externalAudio.autoPlayWarning',
...
class AudioPlayer extends Component {
  constructor(props) {
    ...
    this.opts = {
      ...
      twitch: {
        ...
        playerId: 'externalAudioPlayerTwitch',
      },
      preload: true,
    };
  ...
  componentDidUpdate(prevProp, prevState) {
    // Detect presenter change and redo the sync and listeners to reassign audio to the new one
  ...
  registerVideoListeners() {
    ...
    if (isPresenter) {
      this.syncInterval = setInterval(() => {
        ...
        // Always pause audio if presenter is has not started sharing, e.g., blocked by autoplay
      ...
      onMessage('play', ({ time }) => {
        ...
        logger.debug({ logCode: 'external_audio_client_play' }, 'Play external audio');
      });
      ...
      onMessage('stop', ({ time }) => {
        ...
        logger.debug({ logCode: 'external_audio_client_stop' }, 'Stop external audio');
      });

      onMessage('presenterReady', (data) => {
        const { hasPlayedBefore } = this;

        logger.debug({ logCode: 'external_audio_presenter_ready' }, 'Presenter is ready to sync');
      ...
      onMessage('playerUpdate', (data) => {
        ...
        if (rate !== this.getCurrentPlaybackRate()) {
          ...
          logger.debug({
            logCode: 'external_audio_client_update_rate',
            extraInfo: {
              newRate: rate,
            },
          }, 'Change external audio playback rate.');
        }
  ...
  seekTo(time) {
    ...
    if (Math.abs(this.getCurrentTime() - time) > SYNC_INTERVAL_SECONDS * 0.75) {
      ...
      logger.debug({
        logCode: 'external_audio_client_update_seek',
        extraInfo: { time },
      }, `Seek external audio to: ${time}`);
    }
  }
  ...
  render() {
    const { audioUrl, intl } = this.props;
    const {
...
export default injectIntl(injectWbResizeEvent(AudioPlayer));
